...
function stripQuoted(command: string): string {
 ...
 if (/\b(bash|sh|zsh|ksh|dash)\s+-[a-z]*c\b/i.test(command) || /\beval\b/i.test(command)) {
  return command;
 }
...
function scanRedirectTargets(command: string): string[] {
 ...
 for (const m of command.matchAll(/"[^"]*"/g)) quoteSpans.push([m.index!, m.index! + m[0].length]);
 for (const m of command.matchAll(/'[^']*'/g)) quoteSpans.push([m.index!, m.index! + m[0].length]);
 const wrapped = /\b(bash|sh|zsh|ksh|dash)\s+-[a-z]*c\b/i.test(command) || /\beval\b/i.test(command);
 const inQuote = (i: number) => !wrapped && quoteSpans.some(([s, e]) => i >= s && i < e);
 ...
 for (const m of command.matchAll(reTee)) {
  ...
  for (const tok of rest.matchAll(/"[^"]*"|'[^']*'|\S+/g)) {
   ...
   if (/^[|;&>]/.test(t)) break;
   const isQuote = (t.length >= 2 && t[0] === '"' && t[t.length - 1] === '"') || (t.length >= 2 && t[0] === "'" && t[t.length - 1] === "'");
...
function normalizeTarget(raw: string): string | null {
 ...
 p = p.replace(/\$HOME|\$\{HOME\}/g, HOME).replace(/\$PWD|\$\{PWD\}/g, process.cwd());
 const abs = isAbsolute(p) ? p : resolve(process.cwd(), p);
 return realpathSafe(abs) ?? normalize(abs);
}

// 注意：若主会话被用户命名为 workhorse 也会命中（默认主会话名 undefined，当前惰性）
function isWorkhorseSession(ctx: { sessionManager?: { getSessionName?: () => string | undefined } }): boolean {
 const name = ctx.sessionManager?.getSessionName?.();
 return typeof name === "string" && (name === AGENT_SESSION_PREFIX || name.startsWith(AGENT_SESSION_PREFIX + "#"));
}
 ...
 pi.on("tool_call", async (event, ctx) => {
  ...
  if (event.toolName === "bash") {
   ...
   if (DANGEROUS_PATTERNS.some((re) => re.test(stripQuoted(command)))) {
    return { block: true, reason: "workhorse-gate: 危险命令被阻断（sudo/su、rm -r、chmod/chown 777 及变体）" };
   }
   ...
   for (const raw of scanRedirectTargets(command)) {
    ...
    if (!isUnderHome(target)) {
     return { block: true, reason: `workhorse-gate: bash 重定向目标在 $HOME 之外，已阻断: ${raw}` };
    }
    if (target === PI_AGENT_DIR || target.startsWith(PI_AGENT_DIR + "/")) {
     return { block: true, reason: "workhorse-gate: bash 重定向目标落在 ~/.pi/agent 配置目录，已阻断（规则③）" };
    }
   }
   return undefined;
  }
  ...
  if (event.toolName === "write" || event.toolName === "edit" || event.toolName === "ctx_edit" || event.toolName === "ctx_patch") {
   const raw = event.input.path as string;
   const target = normalizeTarget(raw);
   if (!target) return undefined;

   if (!isUnderHome(target)) {
    return { block: true, reason: `workhorse-gate: 目标路径在 $HOME 之外，已阻断: ${raw}` };
   }
   if (target === PI_AGENT_DIR || target.startsWith(PI_AGENT_DIR + "/")) {
    return { block: true, reason: "workhorse-gate: 禁止修改 ~/.pi/agent 配置目录（规则③）" };
   }
   return undefined;
  }
